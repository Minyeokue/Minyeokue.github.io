...
네임 서버에 관련된 설정(configure; -> conf)을 수정한다습

    [root@Linux01 ~]# vim /etc/named.conf

> 이미지 삽입

주 서버의 영역을 설정해주기 위해 영역 관련 파일을 수정한다.

주 서버의 영역 이름은 gitblog.vm으로 하겠다.
...
이제 Linux01에 웹서버가 잘 작동할 수 있도록 html 파일을 생성하겠다.
...
CentOS7(Linux01)에서 했던 것 처럼 /var/named.conf 파일을 수정한다.
...
이제 ssl 관련 설정 파일 /etc/httpd/conf.d/ssl.conf 수정하도록 한다.
